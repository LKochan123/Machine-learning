...
def process_email(email_contents: str) -> List[int]:
    ...
    inverse_vocabulary_dict = {
        val: key for key, val in vocabulary_dict.items()}
    ...
    email_contents = re.sub(
        '<(http:// | https://)[a-zA-Z]>', 'httpaddr', email_contents)
    ...
    for token in tokens:
        ...
        try:
            word_indices.append(inverse_vocabulary_dict[token])
        except:
            continue
